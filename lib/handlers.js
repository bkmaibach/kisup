...
handlers.sessionCreate = async (data, callback) => {
    //Accept only GET
    if(data.method == 'get'){
        console.log('entered get /sessionCreate');
        //Prepare the data for interpolation - head.title, head.description, body.class, body.description
        let templateData = {
...
handlers.sessionDeleted = async (data, callback) => {
    //Accept only GET
    if(data.method == 'get'){
        console.log('entered get /sessionDeleted');
        //Prepare the data for interpolation - head.title, head.description, body.class, body.description
        let templateData = {
            'head-title' : 'Logged out',
            'head-description' : 'Thanks for dropping by!',
            'body-class' : 'sessionDeleted'
        };
        //Read in the index template as a string
        try{
            let template = await helpers.getTemplate('sessionDeleted', templateData);
            //The interpolation funciton adds the globals automatically
            let pageData = await helpers.addUniversalTemplates(template, templateData);
            callback(200, pageData, 'html');
        } catch (error) {
